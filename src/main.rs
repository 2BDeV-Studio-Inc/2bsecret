...
// v1 = raw SHA-256 KDF (legacy, read-only support)
// v2 = HKDF-SHA256 KDF with domain separation (current)
const VERSION: u8 = 2;
...
fn hkdf_extract_and_expand(shared_secret: &[u8], salt: &[u8], info: &[u8], out_len: usize) -> Vec<u8> {
    // HKDF-Extract: PRK = HMAC(salt, IKM)
    let mut mac = <HmacSha256 as Mac>::new_from_slice(salt).expect("HMAC can take key of any size");
    mac.update(shared_secret);
    let prk = mac.finalize().into_bytes(); // 32 byte
    
    // HKDF-Expand: T(1) = HMAC(PRK, T(0) || info || 0x01)
    ...
    while pos < out_len {
        ...
        mac.update(info);
        mac.update(&[counter]);
        t = mac.finalize().into_bytes().to_vec(); // 32 byte
        
        let to_copy = std::cmp::min(out_len - pos, t.len());
        okm[pos..pos + to_copy].copy_from_slice(&t[..to_copy]);
        pos += to_copy;
...
// Phase 1: HKDF-SHA256 a raw SHA-256 helyett.
// salt = eph_public_bytes (per-encryption egyedi, publikus érték)
// ikm  = DH shared secret
// info = domain separator (verziózott, a jövőbeli protokollváltoztatáshoz)
fn derive_wrap_key_v2(shared_secret: &[u8], eph_public_bytes: &[u8]) -> [u8; 32] {
    let okm = hkdf_extract_and_expand(
        shared_secret,
        eph_public_bytes,                           // salt
        b"2BSecret v2 file-key wrap",               // info
        32,                                         // out_len
    );
    let mut key = [0u8; 32];
...
// Legacy v1: nyers SHA-256 a DH outputra — csak decrypt backward-compat-hoz.
fn derive_wrap_key_v1(shared_secret: &[u8]) -> [u8; 32] {
...
fn generate_device_identity(device_dir: &Path) -> Result<DeviceIdentity> {
    ...
    // [u8; 32] is Copy, tehát `from()` után a seed még a stacken van — explicit töröljük.
    seed.zeroize();
...
fn derive_output_path_for_decrypt(input: &Path, original_name: &str) -> PathBuf {
    ...
    // Végtelenített ciklus a fájlnevekhez
    let mut i = 1;
    loop {
...
fn encrypt_for_recipient(input: &Path, recipient: &RecipientPublic) -> Result<PathBuf> {
    ...
    // Phase 1: HKDF-SHA256 a DH output fölé (domain separation + proper KDF).
    let mut wrap_key_bytes = derive_wrap_key_v2(shared.as_bytes(), eph_public.as_bytes());
    ...
    wrap_key_bytes.zeroize(); // a wrap kulcs a memóriában már nem kell
    ...
    // Strukturáltabb string azonosítók az esetleges későbbi könnyebb parsinghoz
    let header = Header {
...
fn decrypt_on_this_device(input: &Path, device_dir: &Path) -> Result<PathBuf> {
    ...
    // Phase 1: mut + zeroize — DPAPI után a bytes-ok a heap-en vannak, töröljük őket.
    let mut sk_bytes = unprotect_private_key(&protected)?;
    if sk_bytes.len() != 32 { return Err(anyhow!("Invalid unprotected private key length")); }
    let mut sk_arr = [0u8; 32];
    sk_arr.copy_from_slice(&sk_bytes);
    sk_bytes.zeroize(); // a Vec már nem kell, töröljük a heapről
    let secret = StaticSecret::from(sk_arr);
    sk_arr.zeroize(); // Copy típus, ezért marad a stacken from() után is — töröljük
    ...
    // Phase 1: v1 és v2 fájlok is megnyithatók — a KDF ágban döntjük el melyiket használjuk.
    if file_version != 1 && file_version != 2 {
        return Err(anyhow!("Unsupported file version: {}", file_version));
    }
    ...
    // Phase 1: KDF verzióbranch — v1 fájlokhoz legacy SHA-256, v2-höz HKDF.
    let mut wrap_key_bytes: [u8; 32] = match file_version {
        1 => derive_wrap_key_v1(shared.as_bytes()),
        _ => derive_wrap_key_v2(shared.as_bytes(), eph_public.as_bytes()),
    };
    let wrap_cipher = Aes256GcmSiv::new_from_slice(&wrap_key_bytes).map_err(|e| anyhow!("wrap cipher init failed: {e:?}"))?;
    wrap_key_bytes.zeroize(); // wrap kulcs már betöltődött a cipherbe
    let wrap_nonce = Nonce::from_slice(&WRAP_NONCE);
    let wrapped_file_key = B64.decode(&header.wrapped_file_key_b64)?;
    
    // Egységesített side-channel védett hibaüzenet (Kulcs)
    let mut file_key = wrap_cipher.decrypt(wrap_nonce, wrapped_file_key.as_ref())
        .map_err(|_| anyhow!("Decryption failed: wrong recipient, tampered, or corrupted file"))?;
    ...
    loop {
        ...
        // Egységesített side-channel védett hibaüzenet (Tartalom)
        let plaintext = content_cipher.decrypt(nonce, ciphertext.as_ref())
            .map_err(|_| anyhow!("Decryption failed: wrong recipient, tampered, or corrupted file"))?;
